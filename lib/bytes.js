const types = {
  ...
  base64: {
    fromBytes: (bytes) => btoa(String(bytes)), // nodejs
...
const Bytes = {
  ...
  toObj (config, bytes, offset = 0) {
    ...
    for (const field of config) {
      const values = []
      if (field.offset) {
        offset = typeof field.offset === 'function' ? field.offset(obj) : field.offset
      }
      const n = typeof field.n === 'function' ? field.n(obj) : field.n ?? 1
      const length = typeof field.length === 'function' ? field.length(obj) : field.length
      for (let i = 0; i < n; ++i) {
