const types = {
  ...
  str: {
    ...
    toBytes: str => new TextEncoder().encode(str)
  },
  hex: {
    fromBytes: (bytes, { sep = '' } = {}) => [...bytes].map(n => n.toString(16).padStart(2, '0')).join(sep).toLowerCase(),
    toBytes: (hex) => new Uint8Array(Bytes.chunk(hex.replace(/\s/g, ''), 2).map(n => parseInt('0x' + n))),
  },
  base64: {
  ...
  timestamp: {
    // Unix 时间戳, 单位: 秒
    fromBytes: (bytes) => new Date(types.uint32.fromBytes(bytes) * 1000),
    toBytes: (date) => types.uint32.toBytes(date.valueOf() / 1000 | 0),
  },
  uint8: {
...
const Bytes = {
    ...
    for (const field of config) {
      ...
      for (let i = 0; i < n; ++i) {
        ...
        const fieldType = typeof field.type === 'string' ? types[field.type]
          ...
          : field.type
        if (fieldType) {
          values.push(fieldType.fromBytes(data, obj))
        } else {
          values.push(data)
        }
      }
  ...
  reverseBits (bytes) {
    const res = []
    const dict = [0, 8, 4, 12, 2, 10, 6, 14, 1, 9, 5, 13, 3, 11, 7, 15]
    for (let i = bytes.length - 1; i >= 0; --i) {
      const b = bytes[i]
      const lo = dict[b >> 4]
      const hi = dict[b & 0xf] << 4
      res.push(lo | hi)
    }
